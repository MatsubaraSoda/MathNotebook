...
A set $X$, whose elements we shall call _points_, is said to be a _metric space_ if with any two points $p$ and $q$ in $X$ there is associated a real number $d(p,q)$, called the _distance_ from $p$ to $q$, such that
...
If $a_i < b_i$ for $i=1,...,k$, the set of all points $\mathbf{x} = (x_1,...,x_k)$ in $\mathbb{R}^k$ whose coordinates satisfy the inequalities $a_i\leq x_i\leq b_i (1\leq i\leq k)$ is called a _k-cell_. Thus a 1-cell is an interval, a 2-cell is a rectangle, etc.

If $\mathbf{x}\in \mathbb{R}^k$ and $r>0$, the _open_ (or _closed_) _ball_ $B$ with center at $\mathbf{x}$ and radius $r$ is defined to be the set of all $\mathbf{y}\in \mathbb{R}^k$ such that $|\mathbf{y}-\mathbf{x}|<r$ (or $|\mathbf{y}-\mathbf{x}|\leq r$).

We call a set $E\subset \mathbb{R}^k$ _convex_ if
...
For example, _balls are convex_. For if $|\mathbf{y}-\mathbf{x}|<r$, and $|\mathbf{z}-\mathbf{x}|<r$, and $0<\lambda<1$, we have
